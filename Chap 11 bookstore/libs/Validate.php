...
class Validate{
	...
	private function validateNotExistRecord($element, $options){
		...
		$query	  = $options['query'];
		if($database->isExist($query)==true){ // SELECT id FROM user WHERE username  = 'admin' :Nếu tồn tại 1 id nào đó thì coi như đã tồn tại
			$this->setError($element, 'record is exist(Giá trị này đã tồn tại)');
		}
	}
